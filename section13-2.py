...
cursor.execute(
    "CREATE TABLE IF NOT EXISTS records( id INTEGER PRIMARY KEY AUTOINCREMENT, cor_cnt INTEGER,record integer, regdate TEXT)")
...
while n <= 5:
    random.shuffle(words)   # words 리스트를 셔플한다.
    q = random.choice(words)  # 랜덤으로 하나를 뽑아온다.
